...
public class GameController {
	...
	private void initializePieces() throws InvalidColorException, InvalidPositionException {
        createPawns();
        createRooks();
        createKnights();
        createBishops();
	...
	public void createPawns() throws InvalidColorException, InvalidPositionException {
		...
		for (int i = 0; i < 8; i++) {
			Pawn wP = new Pawn(Board.WHITE_PIECE, String.format("%c%d",(char)(firstColumn+i),Board.INITIAL_WHITE_FRONT_ROW));
			board.pieces.put(wP.getPosition(),wP);
			Pawn bP = new Pawn(Board.BLACK_PIECE, String.format("%c%d",(char)(firstColumn + i),Board.INITIAL_BLACK_FRONT_ROW));
	...
	private void createKings() throws InvalidColorException, InvalidPositionException {
       board.pieces.put(Board.INITIAL_WHITE_KING_POSITION, new King(Board.WHITE_PIECE,Board.INITIAL_WHITE_KING_POSITION));
       board.pieces.put(Board.INITIAL_BLACK_KING_POSITION, new King(Board.BLACK_PIECE,Board.INITIAL_BLACK_KING_POSITION));
	}
